...
class GeminiClient:
    ...
    async def generate_stream(self, system: str, prompt: str, temperature: float = 0.2, max_tokens: int = 800):
        if self._client:
            from google.genai import types
            ...
            for model_name in models_to_try:
                try:
                    ...
                    if "resource_exhausted" in err_msg or "quota" in err_msg or "429" in err_msg or "404" in err_msg or "not_found" in err_msg:
                        ...
                        print(f"[gemini] Error streaming with {model_name}: {e}")
                        break

        print("[gemini] Streaming unavailable, buffering via generate()")
        text = await self.generate(system=system, prompt=prompt, temperature=temperature, max_tokens=max_tokens)
        yield text